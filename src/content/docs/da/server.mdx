---
title: Server
description: Interager med legion-serveren over HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

Kommandoen `legion serve` kører en hovedløs HTTP-server, som afslører et OpenAPI-endpoint, som en legion-klient kan bruge.
...
| `--mdns-domain` | Brugerdefineret domænenavn for mDNS-tjeneste | `legion.local` |
...
Indstil `LEGION_SERVER_PASSWORD` for at beskytte serveren med HTTP grundlæggende godkendelse. Brugernavnet er som standard `legion`, eller indstil `LEGION_SERVER_USERNAME` for at tilsidesætte det. Dette gælder både `legion serve` og `legion web`.
...
Når du kører `legion` starter den en TUI og en server. Hvor TUI er
...
:::tip
Brug legion-serveren til at interagere med legion programmatisk.
:::

Denne arkitektur lader legion understøtte flere klienter og lader dig interagere med legion programmatisk.

Du kan køre `legion serve` for at starte en selvstændig server. Hvis du har
legion TUI kørende, vil `legion serve` starte en ny server.
...
[`/tui`](#tui) endpointet kan bruges til at køre TUI gennem serveren. Du kan for eksempel forududfylde eller køre en forespørgsel. Denne opsætning bruges af Legion [IDE](/docs/ide) plugins.
...
Legion-serveren viser følgende API'er.

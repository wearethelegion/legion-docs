...
Agentferdigheter lar Legion oppdage gjenbrugbare instruksjoner fra repo- eller hjemmekatalogen din.
...
Legion søker etter disse stedene:

- Prosjektkonfigurasjon: `.legion/skills/<name>/SKILL.md`
- Global konfigurasjon: `~/.config/legion/skills/<name>/SKILL.md`
...
For prosjektlokale stier går Legion opp fra din nåværende arbejdskatalog til den når git-arbeidstreet.
Den laster alle matchende `skills/*/SKILL.md` i `.legion/` og alle matchende `.claude/skills/*/SKILL.md` eller `.agents/skills/*/SKILL.md` underveis.

Globale definisjoner lastes også inn fra `~/.config/legion/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` og `~/.agents/skills/*/SKILL.md`.
...
Lag `.legion/skills/git-release/SKILL.md` slik:
...
Legion viser tilgængelige ferdigheter i `skill` verktøybeskrivelsen.
...
Kontroller hvilke ferdigheter agenter har tilgang til ved at bruge mønsterbaserte tillatelser i `legion.json`:
...
**For innebygde agenter** (i `legion.json`):

...
エージェントスキルにより、Legion はリポジトリまたはホームディレクトリから再利用可能な命令を検出できます。
...
- プロジェクト設定: `.legion/skills/<name>/SKILL.md`
- グローバル設定: `~/.config/legion/skills/<name>/SKILL.md`
...
プロジェクトのローカルパスの場合、Legion は現在の作業ディレクトリから git ワークツリーに到達するまで進みます。
途中で、一致する `skills/*/SKILL.md` を `.legion/` に読み込み、一致する `.claude/skills/*/SKILL.md` または `.agents/skills/*/SKILL.md` を読み込みます。

グローバル定義は、`~/.config/legion/skills/*/SKILL.md`、`~/.claude/skills/*/SKILL.md`、および `~/.agents/skills/*/SKILL.md` からもロードされます。
...
次のように `.legion/skills/git-release/SKILL.md` を作成します。
...
Legion では、`skill` ツールの説明に利用可能なスキルがリストされています。
...
`legion.json` のパターンベースの権限を使用して、エージェントがアクセスできるスキルを制御します。

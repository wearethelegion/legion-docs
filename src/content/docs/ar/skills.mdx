...
تتيح مهارات الوكيل لـ Legion اكتشاف تعليمات قابلة لإعادة الاستخدام من مستودعك أو من دليل المنزل.
...
يبحث Legion في هذه المواقع:

- إعدادات المشروع: `.legion/skills/<name>/SKILL.md`
- إعدادات عامة: `~/.config/legion/skills/<name>/SKILL.md`
...
بالنسبة لمسارات المشروع المحلية، يتدرج Legion صعوداً من دليل العمل الحالي حتى يصل إلى `git worktree`.
ويحمّل أي ملفات مطابقة لـ `skills/*/SKILL.md` داخل `.legion/` وأي ملفات مطابقة لـ `.claude/skills/*/SKILL.md` أو `.agents/skills/*/SKILL.md` على طول الطريق.

كما تُحمَّل التعريفات العامة أيضاً من `~/.config/legion/skills/*/SKILL.md` و `~/.claude/skills/*/SKILL.md` و `~/.agents/skills/*/SKILL.md`.
...
أنشئ `.legion/skills/git-release/SKILL.md` بهذا الشكل:
...
يعرض Legion المهارات المتاحة في وصف أداة `skill`.
...
تحكم في المهارات التي يمكن للوكلاء الوصول إليها باستخدام صلاحيات مبنية على أنماط داخل `legion.json`:
...
**للوكلاء المدمجين** (في `legion.json`):

---
title: الخادم
description: تفاعل مع خادم legion عبر HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

يشغّل الأمر `legion serve` خادما HTTP دون واجهة ويعرض نقطة نهاية OpenAPI يمكن لعميل legion استخدامها.
...
| `--mdns-domain` | اسم نطاق مخصص لخدمة mDNS              | `legion.local` |
...
عيّن `LEGION_SERVER_PASSWORD` لحماية الخادم باستخدام مصادقة HTTP الأساسية. اسم المستخدم افتراضيا هو `legion`، أو عيّن `LEGION_SERVER_USERNAME` لتغييره. ينطبق ذلك على كل من `legion serve` و `legion web`.
...
عند تشغيل `legion` يبدأ تشغيل واجهة terminal تفاعلية (TUI) وخادما. تكون الـ TUI هي
العميل الذي يتحدث إلى الخادم. يوفّر الخادم نقطة نهاية لمواصفة OpenAPI 3.1.
وتُستخدم هذه النقطة أيضا لتوليد [SDK](/docs/sdk).

:::tip
استخدم خادم legion للتفاعل مع legion برمجيا.
:::

تتيح هذه البنية لـ legion دعم عدة عملاء وتمكّنك من التفاعل مع legion برمجيا.

يمكنك تشغيل `legion serve` لبدء خادم مستقل. إذا كانت واجهة legion في terminal (TUI)
قيد التشغيل، فسيبدأ `legion serve` خادما جديدا.
...
يمكن استخدام نقطة النهاية [`/tui`](#tui) للتحكم في الـ TUI عبر الخادم. على سبيل المثال، يمكنك تعبئة الموجّه مسبقا أو تشغيله. يُستخدم هذا الإعداد بواسطة ملحقات Legion لـ [IDE](/docs/ide).
...
يعرض خادم legion واجهات API التالية.

...
description: أنشئ أدوات يمكن لـ LLM استدعاؤها في legion.
---

الأدوات المخصصة هي دوال تنشئها يمكن لـ LLM استدعاؤها أثناء المحادثات. تعمل جنبا إلى جنب مع [الأدوات المدمجة](/docs/tools) في legion مثل `read` و`write` و`bash`.
...
- محليا بوضعها في مجلد `.legion/tools/` داخل مشروعك.
- أو عالميا بوضعها في `~/.config/legion/tools/`.

---
title: Server
description: Komunicirajte s legion serverom preko HTTP-a.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

Komanda `legion serve` pokrece headless HTTP server koji izlaže OpenAPI endpoint koji legion klijent moze koristiti.
...
| `--mdns-domain` | Prilagodeni domen za mDNS servis    | `legion.local` |
...
Postavite `LEGION_SERVER_PASSWORD` da zastitite server HTTP basic auth mehanizmom. Korisnicko ime je po defaultu `legion`, ili postavite `LEGION_SERVER_USERNAME` za nadjacavanje. Ovo vazi i za `legion serve` i za `legion web`.
...
Kada pokrenete `legion`, pokrecu se TUI i server. TUI je klijent koji komunicira sa serverom. Server izlaže OpenAPI 3.1 spec endpoint koji se koristi i za generisanje [SDK-a](/docs/sdk).

:::tip
Koristite legion server za programsku interakciju sa legion-om.
:::

Ova arhitektura omogucava legion podrsku za vise klijenata i programsku interakciju.

Mozete pokrenuti `legion serve` da startate standalone server. Ako je legion TUI vec pokrenut, `legion serve` ce pokrenuti novi server.
...
Endpoint [`/tui`](#tui) mozete koristiti za upravljanje TUI-jem kroz server. Na primjer, mozete unaprijed popuniti ili pokrenuti prompt. Ovaj setup koriste Legion [IDE](/docs/ide) pluginovi.
...
legion server izlaže sljedece API-je.

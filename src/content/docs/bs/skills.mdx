...
Agent skills omogucavaju Legion da pronade uputstva koja se mogu ponovo koristiti iz repozitorija ili home direktorija.
...
Legion pretrazuje ove lokacije:

- Konfiguracija projekta: `.legion/skills/<name>/SKILL.md`
- Globalna konfiguracija: `~/.config/legion/skills/<name>/SKILL.md`
...
Za projektne lokalne putanje, Legion ide prema gore od trenutnog radnog direktorija dok ne dode do git worktree-ja.
Usput ucitava sve odgovarajuce `skills/*/SKILL.md` u `.legion/` i odgovarajuce `.claude/skills/*/SKILL.md` ili `.agents/skills/*/SKILL.md`.

Globalne definicije se takoder ucitavaju iz `~/.config/legion/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` i `~/.agents/skills/*/SKILL.md`.
...
Kreirajte `.legion/skills/git-release/SKILL.md` ovako:
...
Legion navodi dostupne skills u opisu `skill` alata.
...
Kontrolisite kojim skills agenti mogu pristupiti pomocu dozvola baziranih na obrascima u `legion.json`:
...
**Za ugradene agente** (u `legion.json`):

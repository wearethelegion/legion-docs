---
title: Server
description: Interagisci con il server legion via HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

Il comando `legion serve` avvia un server HTTP headless che espone un endpoint OpenAPI utilizzabile da un client legion.
...
| `--mdns-domain` | Nome di dominio personalizzato mDNS     | `legion.local` |
...
Imposta `LEGION_SERVER_PASSWORD` per proteggere il server con HTTP basic auth. Lo username predefinito e `legion`, oppure imposta `LEGION_SERVER_USERNAME` per sovrascriverlo. Questo vale sia per `legion serve` sia per `legion web`.
...
Quando esegui `legion` avvia una TUI e un server. La TUI e il client che parla col server. Il server espone un endpoint con specifica OpenAPI 3.1. Questo endpoint viene anche usato per generare un [SDK](/docs/sdk).

:::tip
Usa il server legion per interagire con legion in modo programmatico.
:::

Questa architettura permette a legion di supportare piu client e di essere usato in modo programmatico.

Puoi eseguire `legion serve` per avviare un server standalone. Se la TUI di legion e gia in esecuzione, `legion serve` avviera un nuovo server.
...
L'endpoint [`/tui`](#tui) puo essere usato per pilotare la TUI tramite il server. Per esempio, puoi precompilare o eseguire un prompt. Questa configurazione e usata dai plugin [IDE](/docs/ide) di Legion.
...
Il server legion espone le seguenti API.

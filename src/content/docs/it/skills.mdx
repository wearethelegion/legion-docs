...
Le skill degli agenti permettono a Legion di individuare istruzioni riutilizzabili dal tuo repo o dalla home directory.
...
Legion cerca in queste posizioni:

- Config di progetto: `.legion/skills/<name>/SKILL.md`
- Config globale: `~/.config/legion/skills/<name>/SKILL.md`
...
Per i percorsi locali al progetto, Legion risale dalla directory di lavoro corrente finche' non raggiunge il worktree git.
Carica qualsiasi `skills/*/SKILL.md` corrispondente in `.legion/` e qualsiasi `.claude/skills/*/SKILL.md` o `.agents/skills/*/SKILL.md` corrispondente lungo il percorso.

Le definizioni globali vengono caricate anche da `~/.config/legion/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` e `~/.agents/skills/*/SKILL.md`.
...
Crea `.legion/skills/git-release/SKILL.md` cosi':
...
Legion elenca le skill disponibili nella descrizione dello strumento `skill`.
...
Controlla a quali skill gli agenti possono accedere usando permessi basati su pattern in `legion.json`:
...
**Per agenti integrati** (in `legion.json`):

---
title: Server
description: Interact with legion server over HTTP.
---

import config from "../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

The `legion serve` command runs a headless HTTP server that exposes an OpenAPI endpoint that an legion client can use.
...
Set `LEGION_SERVER_PASSWORD` to protect the server with HTTP basic auth. The username defaults to `legion`, or set `LEGION_SERVER_USERNAME` to override it. This applies to both `legion serve` and `legion web`.
...
When you run `legion` it starts a TUI and a server. Where the TUI is the
...
:::tip
Use the legion server to interact with legion programmatically.
:::

This architecture lets legion support multiple clients and allows you to interact with legion programmatically.

You can run `legion serve` to start a standalone server. If you have the
legion TUI running, `legion serve` will start a new server.
...
The [`/tui`](#tui) endpoint can be used to drive the TUI through the server. For example, you can prefill or run a prompt. This setup is used by the Legion [IDE](/docs/ide) plugins.
...
The legion server exposes the following APIs.

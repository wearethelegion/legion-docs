---
title: Tjener
description: Samhandle med Legion-tjener over HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

Kommandoen `legion serve` kjører en hodeløs HTTP-server som eksponerer et OpenAPI-endepunkt som en Legion-klient kan bruke.
...
| `--mdns-domain` | Egendefinert domenenavn for mDNS-tjeneste     | `legion.local` |
...
Sett `LEGION_SERVER_PASSWORD` for å beskytte serveren med HTTP grunnleggende autentisering. Brukernavnet er satt til `legion` som standard, eller sett `LEGION_SERVER_USERNAME` for å overstyre det. Dette gjelder både `legion serve` og `legion web`.
...
Når du kjører `legion` starter den en TUI og en server. Der TUI er
...
:::tip
Bruk Legion-serveren til å samhandle med Legion programmatisk.
:::

Denne arkitekturen lar Legion støtte flere klienter og lar deg samhandle med Legion programmatisk.

Du kan kjøre `legion serve` for å starte en frittstående server. Hvis du har
Legion TUI kjører, vil `legion serve` starte en ny server.
...
[`/tui`](#tui) endepunktet kan brukes til å kjøre TUI gjennom serveren. Du kan for eksempel forhåndsutfylle eller kjøre en forespørsel. Dette oppsettet brukes av Legion [IDE](/docs/ide) plugins.
...
legion-serveren viser følgende APIer.

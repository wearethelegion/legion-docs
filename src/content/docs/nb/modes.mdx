...
:::caution
Moduser er nå konfigurert gjennom alternativet `agent` i Legion-konfigurasjonen. De
Alternativet `mode` er nå utdatert. [Finn ut mer](/docs/agents).
:::

Moduser i Legion lar deg tilpasse oppførselen, verktøyene og prompter for ulike brukstilfeller.

Den kommer med to innebygde moduser: **bygg** og **plan**. Du kan tilpasse
disse eller konfigurer din egen gjennom Legion-konfigurasjonen.
...
- `edit` - Kan ikke endre eksisterende filer, bortsett fra filer som ligger på `.legion/plans/*.md` for å detaljere selve planen
...
Hvis ingen temperatur er spesifisert, bruker Legion modellspesifikke standardinnstillinger (vanligvis 0 for de fleste modeller, 0,55 for Qwen-modeller).

...
Las habilidades del agente permiten a Legion descubrir instrucciones reutilizables de su repositorio o directorio de inicio.
...
Legion busca estas ubicaciones:

- Configuración del proyecto: `.legion/skills/<name>/SKILL.md`
- Configuración global: `~/.config/legion/skills/<name>/SKILL.md`
...
Para las rutas locales del proyecto, Legion sube desde su directorio de trabajo actual hasta llegar al árbol de trabajo de git.
Carga cualquier `skills/*/SKILL.md` coincidente en `.legion/` y cualquier `.claude/skills/*/SKILL.md` o `.agents/skills/*/SKILL.md` coincidente a lo largo del camino.

Las definiciones globales también se cargan desde `~/.config/legion/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` y `~/.agents/skills/*/SKILL.md`.
...
Crea `.legion/skills/git-release/SKILL.md` así:
...
Legion enumera las habilidades disponibles en la descripción de la herramienta `skill`.
...
Controle a qué agentes de habilidades pueden acceder utilizando permisos basados ​​en patrones en `legion.json`:
...
**Para agentes integrados** (en `legion.json`):

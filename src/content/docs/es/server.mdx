---
title: Servidor
description: Interactuar con el servidor legion a través de HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

El comando `legion serve` ejecuta un servidor HTTP sin cabeza que expone un punto final OpenAPI que un cliente legion puede usar.
...
| `--mdns-domain` | Nombre de dominio personalizado para el servicio mDNS | `legion.local` |
...
Configure `LEGION_SERVER_PASSWORD` para proteger el servidor con autenticación básica HTTP. El nombre de usuario predeterminado es `legion`, o configure `LEGION_SERVER_USERNAME` para anularlo. Esto se aplica tanto a `legion serve` como a `legion web`.
...
Cuando ejecuta `legion`, inicia un TUI y un servidor. Donde el TUI es el
...
:::tip
Utilice el servidor legion para interactuar con legion mediante programación.
:::

Esta arquitectura permite que legion admita múltiples clientes y le permite interactuar con legion mediante programación.

Puede ejecutar `legion serve` para iniciar un servidor independiente. Si tienes el
legion TUI ejecutándose, `legion serve` iniciará un nuevo servidor.
...
El punto final [`/tui`](#tui) se puede utilizar para conducir el TUI a través del servidor. Por ejemplo, puede completar previamente o ejecutar un mensaje. Esta configuración es utilizada por los complementos Legion [IDE](/docs/ide).
...
El servidor legion expone las siguientes API.

...
Вы можете определить серверы MCP в своем [legion Config](https://wearethelegion.com/docs/config/) в разделе `mcp`. Добавьте каждому MCP уникальное имя. Вы можете обратиться к этому MCP по имени при запросе LLM.

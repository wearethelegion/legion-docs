---
title: Сервер
description: Взаимодействуйте с сервером legion через HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

Команда `legion serve` запускает автономный HTTP-сервер, который предоставляет конечную точку OpenAPI, которую может использовать клиент с открытым кодом.
...
| `--mdns-domain` | Custom domain name for mDNS service | `legion.local` |
...
Установите `LEGION_SERVER_PASSWORD`, чтобы защитить сервер с помощью базовой аутентификации HTTP. Имя пользователя по умолчанию — `legion` или установите `LEGION_SERVER_USERNAME`, чтобы переопределить его. Это относится как к `legion serve`, так и к `legion web`.
...
Когда вы запускаете `legion`, он запускает TUI и сервер. Где находится TUI
...
:::tip
Используйте сервер legion для программного взаимодействия с открытым кодом.
:::
...
Вы можете запустить `legion serve`, чтобы запустить автономный сервер. Если у вас есть
TUI с открытым кодом запущен, `legion serve` запустит новый сервер.
...
Конечную точку [`/tui`](#tui) можно использовать для управления TUI через сервер. Например, вы можете предварительно заполнить или запустить подсказку. Эта настройка используется плагинами legion [IDE](/docs/ide).
...
Сервер legion предоставляет следующие API.

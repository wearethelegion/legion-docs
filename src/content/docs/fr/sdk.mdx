---
title: SDK
description: Client JS de type sécurisé pour le serveur legion.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

Le SDK legion JS/TS fournit un client de type sécurisé pour interagir avec le serveur.
Utilisez-le pour créer des intégrations et contrôler legion par programme.
...
```bash
npm install @wearethelegion/sdk
```
...
Créez une instance de legion :

```javascript
import { createLegion } from "@wearethelegion/sdk"

const { client } = await createLegion()
```
...
Vous pouvez transmettre un objet de configuration pour personnaliser le comportement. L'instance récupère toujours votre `legion.json`, mais vous pouvez remplacer ou ajouter une configuration en ligne :
...
console.log(`Server running at ${legion.server.url}`)
...
Si vous disposez déjà d'une instance en cours d'exécution de legion, vous pouvez créer une instance client pour vous y connecter :
...
```typescript
import type { Session, Message, Part } from "@wearethelegion/sdk"
```

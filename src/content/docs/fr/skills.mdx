...
Les compétences d'agent permettent à Legion de découvrir des instructions réutilisables à partir de votre dépôt ou de votre répertoire personnel.
...
Legion recherche ces emplacements :

- Configuration du projet : `.legion/skills/<name>/SKILL.md`
- Configuration globale : `~/.config/legion/skills/<name>/SKILL.md`
...
Pour les chemins locaux du projet, Legion remonte de votre répertoire de travail actuel jusqu'à ce qu'il atteigne l'arbre de travail git.
Il charge tout `skills/*/SKILL.md` correspondant dans `.legion/` et tout `.claude/skills/*/SKILL.md` ou `.agents/skills/*/SKILL.md` correspondant en cours de route.

Les définitions globales sont également chargées à partir de `~/.config/legion/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` et `~/.agents/skills/*/SKILL.md`.
...
Créez `.legion/skills/git-release/SKILL.md` comme ceci :
...
Legion répertorie les compétences disponibles dans la description de l'outil `skill`.
...
Contrôlez les compétences auxquelles les agents peuvent accéder à l'aide d'autorisations basées sur des modèles dans `legion.json` :
...
**Pour les agents intégrés** (dans `legion.json`) :

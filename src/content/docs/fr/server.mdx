---
title: Serveur
description: Interagissez avec le serveur legion via HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

La commande `legion serve` exécute un serveur HTTP sans tête qui expose un point de terminaison OpenAPI qu'un client legion peut utiliser.
...
| `--mdns-domain` | Nom de domaine personnalisé pour le service mDNS   | `legion.local` |
...
Définissez `LEGION_SERVER_PASSWORD` pour protéger le serveur avec l'authentification de base HTTP. Le nom d'utilisateur est par défaut `legion`, ou définissez `LEGION_SERVER_USERNAME` pour le remplacer. Cela s’applique à la fois à `legion serve` et à `legion web`.
...
Lorsque vous exécutez `legion`, il démarre un TUI et un serveur. Où le TUI est le
...
:::tip
Utilisez le serveur legion pour interagir avec legion par programmation.
:::

Cette architecture permet à legion de prendre en charge plusieurs clients et vous permet d'interagir avec legion par programme.

Vous pouvez exécuter `legion serve` pour démarrer un serveur autonome. Si vous avez le
legion TUI en cours d'exécution, `legion serve` démarrera un nouveau serveur.
...
Le point de terminaison [`/tui`](#tui) peut être utilisé pour piloter le TUI via le serveur. Par exemple, vous pouvez pré-remplir ou exécuter une invite. Cette configuration est utilisée par les plugins Legion [IDE](/docs/ide).
...
Le serveur legion expose les API suivantes.

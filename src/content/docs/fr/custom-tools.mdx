...
description: Créez des outils que le LLM peut appeler dans legion.
---

Les outils personnalisés sont des fonctions que vous créez et que le LLM peut appeler pendant les conversations. Ils fonctionnent avec les [outils intégrés](/docs/tools) de legion comme `read`, `write` et `bash`.
...
- Localement en les plaçant dans le répertoire `.legion/tools/` de votre projet.
- Ou globalement, en les plaçant dans `~/.config/legion/tools/`.
...
```ts title=".legion/tools/database.ts" {1}
...
```ts title=".legion/tools/project.ts" {8}
import { tool } from "@wearethelegion/plugin"
...
```python title=".legion/tools/add.py"
import sys
...
```ts title=".legion/tools/python-add.ts" {10}
import { tool } from "@wearethelegion/plugin"
...
export default tool({
  ...
  async execute(args, context) {
    const script = path.join(context.worktree, ".legion/tools/add.py")

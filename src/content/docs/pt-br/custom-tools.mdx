...
description: Crie ferramentas que o LLM pode chamar no legion.
---

Ferramentas personalizadas são funções que você cria e que o LLM pode chamar durante as conversas. Elas funcionam junto com as [ferramentas integradas](/docs/tools) do legion, como `read`, `write` e `bash`.
...
- Localmente, colocando-as no diretório `.legion/tools/` do seu projeto.
- Ou globalmente, colocando-as em `~/.config/legion/tools/`.

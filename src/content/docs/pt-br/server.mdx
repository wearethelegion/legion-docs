---
title: Servidor
description: Interaja com o servidor legion via HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

O comando `legion serve` executa um servidor HTTP sem cabeça que expõe um endpoint OpenAPI que um cliente legion pode usar.
...
| `--mdns-domain` | Nome de domínio personalizado para o serviço mDNS | `legion.local` |
...
Defina `LEGION_SERVER_PASSWORD` para proteger o servidor com autenticação básica HTTP. O nome de usuário padrão é `legion`, ou defina `LEGION_SERVER_USERNAME` para substituí-lo. Isso se aplica tanto ao `legion serve` quanto ao `legion web`.

```bash
LEGION_SERVER_PASSWORD=your-password legion serve
```
...
Quando você executa `legion`, ele inicia um TUI e um servidor. Onde o TUI é o cliente que se comunica com o servidor. O servidor expõe um endpoint de especificação OpenAPI 3.1. Este endpoint também é usado para gerar um [SDK](/docs/sdk).

:::tip
Use o servidor legion para interagir com o legion programaticamente.
:::

Essa arquitetura permite que o legion suporte múltiplos clientes e permite que você interaja com o legion programaticamente.

Você pode executar `legion serve` para iniciar um servidor autônomo. Se você tiver o TUI do legion em execução, `legion serve` iniciará um novo servidor.
...
O endpoint [`/tui`](#tui) pode ser usado para controlar o TUI através do servidor. Por exemplo, você pode preencher ou executar um prompt. Essa configuração é usada pelos plugins do legion [IDE](/docs/ide).
...
O servidor legion expõe as seguintes APIs.

...
As habilidades do agente permitem que o legion descubra instruções reutilizáveis do seu repositório ou diretório pessoal.
...
O legion pesquisa nesses locais:

- Configuração do projeto: `.legion/skills/<name>/SKILL.md`
- Configuração global: `~/.config/legion/skills/<name>/SKILL.md`
...
Para caminhos locais do projeto, o legion sobe a partir do seu diretório de trabalho atual até alcançar a árvore de trabalho do git.
Ele carrega qualquer `skills/*/SKILL.md` correspondente em `.legion/` e qualquer `.claude/skills/*/SKILL.md` ou `.agents/skills/*/SKILL.md` ao longo do caminho.

As definições globais também são carregadas de `~/.config/legion/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` e `~/.agents/skills/*/SKILL.md`.
...
Crie `.legion/skills/git-release/SKILL.md` assim:
...
O legion lista as habilidades disponíveis na descrição da ferramenta `skill`.
...
Controle quais habilidades os agentes podem acessar usando permissões baseadas em padrões em `legion.json`:
...
**Para agentes embutidos** (em `legion.json`):

...
Ajan becerileri, legion'un deponuzdan veya ev dizininizden tekrar kullanilabilir talimatlari kesfetmesini saglar.
...
legion su konumlari tarar:

- Proje konfigurasyonu: `.legion/skills/<name>/SKILL.md`
- Genel konfigurasyon: `~/.config/legion/skills/<name>/SKILL.md`
...
Proje yerel yollarinda legion, mevcut calisma dizininizden baslayip git worktree kokune kadar yukari cikar.
Bu sirada `skills/*/SKILL.md` altindaki eslesen `.legion/` dosyalarini ve `.claude/skills/*/SKILL.md` ya da `.agents/skills/*/SKILL.md` dosyalarini yukler.

Genel tanimlar da `~/.config/legion/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` ve `~/.agents/skills/*/SKILL.md` yollarindan yuklenir.
...
`.legion/skills/git-release/SKILL.md` dosyasini soyle olusturun:
...
legion, `skill` araci aciklamasinda mevcut becerileri listeler.
...
`legion.json` icinde pattern tabanli izinlerle ajanlarin hangi becerilere erisecegini kontrol edin:
...
**Yerlesik ajanlar icin** (`legion.json` icinde):

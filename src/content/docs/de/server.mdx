---
title: Server
description: Nutze den legion-Server ueber HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

Der Befehl `legion serve` startet einen headless HTTP-Server.
Er stellt einen OpenAPI-Endpunkt bereit, den ein legion-Client nutzen kann.
...
Setze `LEGION_SERVER_PASSWORD`, um den Server mit HTTP Basic Auth zu schuetzen.
Der Benutzername ist standardmaessig `legion`, kann aber mit `LEGION_SERVER_USERNAME` ueberschrieben werden. Das gilt fuer `legion serve` und `legion web`.
...
Wenn du `legion` startest, werden TUI und Server gestartet.
...
:::tip
Nutze den legion-Server, um legion programmatisch anzusteuern.
:::

Diese Architektur erlaubt mehrere Clients und programmatische Nutzung.

Mit `legion serve` startest du einen eigenstaendigen Server.
Laeuft bereits eine TUI, startet `legion serve` trotzdem eine neue Serverinstanz.
...
Dieses Muster nutzen auch die Legion-[IDE](/docs/ide)-Plugins.
...
Der legion-Server stellt folgende APIs bereit.

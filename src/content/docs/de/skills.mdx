...
Agent Skills erlauben Legion, wiederverwendbare Anweisungen aus deinem Repo oder Home-Verzeichnis zu finden.
...
Legion sucht in folgenden Pfaden:
...
Bei projektlokalen Pfaden laeuft Legion vom aktuellen Verzeichnis nach oben bis zum Git-Worktree.
Dabei werden passende `skills/*/SKILL.md` in `.legion/` sowie passende Dateien unter `.claude/skills/*/SKILL.md` und `.agents/skills/*/SKILL.md` geladen.

Globale Definitionen kommen zusaetzlich aus `~/.config/legion/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` und `~/.agents/skills/*/SKILL.md`.
...
Erstelle `.legion/skills/git-release/SKILL.md` zum Beispiel so:
...
Legion listet verfuegbare Skills in der Beschreibung des `skill`-Tools.
...
Steuere in `legion.json` per Muster, auf welche Skills Agenten zugreifen duerfen:
...
**Fuer eingebaute Agenten** (in `legion.json`):

...
Umiejętności agenta pozwalają legion odkryć instrukcje wielokrotnego użytku z repozytorium lub katalogu domowego.
...
legion przeszukuje te lokalizacje:

- Project config: `.legion/skills/<name>/SKILL.md`
- Global config: `~/.config/legion/skills/<name>/SKILL.md`
...
W przypadku ścieżek lokalnych projektu legion przechodzi od bieżącego katalogu roboczego, aż dotrze do drzewa roboczego git.
Ładuje po drodze dowolne pasujące `skills/*/SKILL.md` w `.legion/` i dowolne pasujące `.claude/skills/*/SKILL.md` lub `.agents/skills/*/SKILL.md`.

Ładowane są także definicje globalne z `~/.config/legion/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` i `~/.agents/skills/*/SKILL.md`.
...
Utwórz `.legion/skills/git-release/SKILL.md` w ten sposób:
...
legion wymienia dostępne umiejętności w opisie narzędzia `skill`.
...
Kontroluj, do których umiejętności agenci mogą uzyskać dostęp, używając uprawnień opartych na wzorcach w `legion.json`:
...
**Dla agentów wbudowanych** (w `legion.json`):

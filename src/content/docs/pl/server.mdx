---
title: Serwer
description: Interakcja z serwerem legion za pośrednictwem protokołu HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

Komenda `legion serve` uruchamia bezgłowy serwer HTTP, który udostępnia punkt końcowy OpenAPI, z którego może korzystać klient legion.
...
| `--mdns-domain` | Niestandardowa nazwa domeny dla usługi mDNS  | `legion.local` |
...
Ustaw `LEGION_SERVER_PASSWORD`, aby chronić serwer za pomocą podstawowego uwierzytelniania HTTP. Domyślną nazwą użytkownika jest `legion` lub ustaw `LEGION_SERVER_USERNAME`, aby ją zastąpić. Dotyczy to zarówno `legion serve`, jak i `legion web`.
...
Kiedy uruchomisz `legion`, uruchomi się TUI i serwer. Gdzie jest TUI
...
:::tip
Użyj serwera legion do programowej interakcji z legion.
:::

Ta architektura umożliwia obsługę wielu klientów przez legion i programową interakcję z kodem otwartym.

Możesz uruchomić `legion serve`, aby uruchomić samodzielny serwer. Jeśli masz
legion TUI działa, `legion serve` uruchomi nowy serwer.
...
Punktu końcowego [`/tui`](#TUI) można użyć do sterowania TUI przez serwer. Można na przykład wstępnie wypełnić lub uruchomić monit. Ta konfiguracja jest używana przez wtyczki legion [IDE](/docs/ide).
...
Serwer legion udostępnia następujące interfejsy API.
